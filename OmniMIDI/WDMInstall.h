...
/* 
 * Code taken from: https://github.com/munt/munt/blob/master/mt32emu_win32drv_setup/drvsetup/drvsetup.cpp
 * 
...
const char OMNIMIDI_DRIVER_NAME[] = "OmniMIDI.dll";
...
const char INSTALL_COMMAND[] = "RegisterDrv";
const char UNINSTALL_COMMAND[] = "UnregisterDrv";
const char REPAIR_COMMAND[] = "RepairDrv";
...
const char DEVICE_DESCRIPTION[] = "OmniMIDI for Windows NT";
const char DRIVER_PROVIDER_NAME[] = "Keppy's Software";
const char DEVICE_HARDWARE_IDS[] = "ROOT\\OmniMIDI\0";
...
const char DRIVER_SUBCLASS_SUBKEYS[] = "MIDI\\OmniMIDI.dll";
...
const char SUCCESSFULLY_INSTALLED_MSG[] = "OmniMIDI successfully installed";
const char SUCCESSFULLY_UPDATED_MSG[] = "OmniMIDI successfully updated";
const char SUCCESSFULLY_UNINSTALLED_MSG[] = "OmniMIDI successfully uninstalled";
const char SUCCESSFULLY_REPAIRED_MSG[] = "OmniMIDI successfully repaired";
...
const char CANNOT_INSTALL_NO_PORTS_ERR[] = "Cannot install OmniMIDI:\n There is no MIDI ports available";
...
const char CANNOT_UNINSTALL_ERR[] = "Cannot uninstall OmniMIDI";
const char CANNOT_UNINSTALL_32_ERR[] = "Cannot uninstall 32-bit OmniMIDI";
const char CANNOT_UNINSTALL_NOT_FOUND_ERR[] = "Cannot uninstall OmniMIDI:\n There is no driver registry entry found";
const char CANNOT_INSTALL_PATH_TOO_LONG_ERR[] = "OmniMIDI cannot be installed:\n Installation path is too long";
const char CANNOT_INSTALL_FILE_COPY_ERR[] = "OmniMIDI failed to install:\n File copying error";
const char CANNOT_INSTALL_32_FILE_COPY_ERR[] = "32-bit OmniMIDI failed to install:\n File copying error";
...
typedef WINBASEAPI BOOL(WINAPI* vIsWow64Process)(_In_ HANDLE hProcess, _Out_ PBOOL wow64Process);

// Function IsWow64Process() is only available since _WIN32_WINNT 0x0501, so it is linked lazily.
static bool isWow64Process() {
	const vIsWow64Process fnIsWow64Process = (vIsWow64Process)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "IsWow64Process");
...
static bool findFreeMidiRegEntry(int& entryIx, HKEY hReg, MidiRegistryEntryName& entryName) {
	...
	for (int i = 0; i < 10; i++) {
		...
		if (_stricmp(str, OMNIMIDI_DRIVER_NAME) == 0) {
			entryIx = i;
			return false;
		}
		if (freeEntryIx == -1) {
...
static int findMt32emuRegEntry(HKEY hReg, MidiRegistryEntryName& entryName) {
	...
	for (int i = 0; i < 10; i++) {
		DWORD len = 255;
		LSTATUS res = RegQueryValueExA(hReg, entryName.withIndex(i)->toCString(), NULL, NULL, (LPBYTE)str, &len);
		if (res == ERROR_SUCCESS && _stricmp(str, OMNIMIDI_DRIVER_NAME) == 0) {
			return i;
		}
	}
	return -1;
...
static bool registerDriverInWow(const char* mt32emuEntryName) {
	...
	LSTATUS res = RegSetValueExA(hReg, mt32emuEntryName, NULL, REG_SZ, (LPBYTE)OMNIMIDI_DRIVER_NAME, sizeof(OMNIMIDI_DRIVER_NAME));
	RegCloseKey(hReg);
	if (res != ERROR_SUCCESS) {
...
static void unregisterDriverInWow(const char* mt32emuEntryName) {
	...
	if (res != ERROR_SUCCESS || _stricmp(str, OMNIMIDI_DRIVER_NAME)) {
...
static void constructDriverPathName(char* pathName, const char* systemDirName) {
	...
	strncat(pathName, OMNIMIDI_DRIVER_NAME, MAX_PATH - strlen(pathName));
}
...
static void deleteFileReliably(char* pathName, const char* systemDirName) {
	...
	char tmpFilePrefix[sizeof(OMNIMIDI_DRIVER_NAME) + 1];
	strncpy(tmpFilePrefix, OMNIMIDI_DRIVER_NAME, sizeof(OMNIMIDI_DRIVER_NAME));
...
static bool registerDriverClass(HKEY devRegKey, int legacyMidiEntryIx) {
	...
	RegCloseKey(driversSubkey);
	if (ERROR_SUCCESS != RegSetValueExA(driverSubkey, DRIVER_SUBCLASS_PROP_DRIVER, NULL, REG_SZ, (LPBYTE)OMNIMIDI_DRIVER_NAME, sizeof(OMNIMIDI_DRIVER_NAME))) {
...
inline void WINAPI DriverRegistration(HWND HWND, HINSTANCE HinstanceDLL, LPSTR CommandLine, DWORD CmdShow) {
	int legacyMidiEntryIx;
	bool wow64Process = isWow64Process();
	OperationMode mode = OperationMode_UNKNOWN;

	if (_stricmp(INSTALL_COMMAND, CommandLine) == 0) {
		mode = OperationMode_INSTALL;
	}
	else if (_stricmp(UNINSTALL_COMMAND, CommandLine) == 0) {
		mode = OperationMode_UNINSTALL;
	}
	else if (_stricmp(REPAIR_COMMAND, CommandLine) == 0) {
	...
	case OperationMode_INSTALL:
	{
		...
		if (registerDriverResult == RegisterDriverResult_FAILED) {
			MessageBoxA(NULL, "An error has occurred while running registerDriver", INFORMATION_TITLE, MB_OK | MB_ICONINFORMATION);
			return;
		}
		...
		if (ProcessReturnCode_OK != returnCode) {
			MessageBoxA(NULL, "An error has occurred while running registerDeviceAndDriverClass", INFORMATION_TITLE, MB_OK | MB_ICONINFORMATION);
			return;
		}
	...
	case OperationMode_UNINSTALL:
	{
		...
		if (!unregisterDriver(wow64Process))
			return;

		ProcessReturnCode returnCode = removeDevice(wow64Process);
		if (ProcessReturnCode_OK != returnCode) {
			MessageBoxA(NULL, "An error has occurred while running removeDevice", INFORMATION_TITLE, MB_OK | MB_ICONINFORMATION);
			return;
		}
	...
	case OperationMode_REPAIR:
	{
		...
		if (registerDriverResult == RegisterDriverResult_OK)
			MessageBoxA(NULL, SUCCESSFULLY_REPAIRED_MSG, INFORMATION_TITLE, MB_OK | MB_ICONINFORMATION);
