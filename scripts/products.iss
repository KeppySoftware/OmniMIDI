...
begin
  ...
  begin
    if WizardForm.TasksList.Checked[2] then
      ExtractTemporaryFile('LoudMax.dll');
      FileCopy(ExpandConstant('{tmp}\LoudMax.dll'), ExpandConstant('{%USERPROFILE}\Keppy''s Synthesizer\LoudMax.dll'), False);
      DeleteFile(ExpandConstant('{tmp}\LoudMax.dll'));
  end;
